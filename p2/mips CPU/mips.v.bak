module mips(clk,rst,PrAddr,PrDIn,PrDOut,Wen,HWInt);
  ...
  ctrl CU(.instruction(instruction),.RegDst(RegDst),.RegWr(RegWr),.ExtOp(ExtOp),.nPC_sel(nPC_sel),.ALUctr(ALUctr),.MemtoReg(MemtoReg),.MemWr(MemWr),.ALUSrc(ALUSrc),.j_sel(j_sel),.jal_sel(jal_sel),.lb_sel(lb_sel),.sb_sel(sb_sel),.PCWr(PCWr),.IRWr(IRWr),.clk(clk),.rst(rst));
  ...
  .jal_sel(jal_sel),.lb_sel(lb_sel),.sb_sel(sb_sel),.PCWr(PCWr),.IRWr(IRWr),.PrRD(PrDIn),.PrWD(PrDOut),.PrAddr(PrAddr));
endmodule
